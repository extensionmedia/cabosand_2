<div class="mb-20 bg-green-50 rounded-lg p-4 border border-green-400">
	<div class="flex pb-5 gap-1 w-full">
		<div class="w-32">
			...
			<input type="date" id="periode_de" class="font-bold" style="font-size: 10px;" value="<?= $ppl["de"] ?>">
			<input type="hidden" id="id" value="<?= $ppl["id"] ?>">
		</div>
		<div class="w-32">
			<label for="periode_a">Date Fin </label>
			<input type="date" id="periode_a" class="font-bold" style="font-size: 10px;" value="<?=$ppl["a"]?>">
		</div>
		<div class="w-24">
			<label for="periode_nuite">Nuités </label>
			<input type="number" id="periode_nuite" class="font-bold" style="font-size: 10px;" value="<?=$ppl["nbr_nuite"]?>" style="font-size: 16px; text-align: center; background-color: rgba(249,245,191,1.00)">
		</div>
	</div>
	<div class="flex gap-1 pb-10">
		<div class="w-32">
			<label for="periode_montant">Montant</label>
			<input type="number" class="font-bold" style="font-size: 10px;" id="periode_montant" value="<?=$ppl["montant"]?>">
		</div>
		<div class="flex-1">
			<label for="date_fin">Type </label>
			<select id="ppl_type" class="font-bold" style="font-size: 10px;">
				<option selected value="-1"></option>
				<option <?= $ppl["id_propriete_location_type"] === "1"? "selected": "" ?> value="1">Par Nuit</option>
				<option <?= $ppl["id_propriete_location_type"] === "2"? "selected": "" ?> value="2">Par Mois</option>
				<option <?= $ppl["id_propriete_location_type"] === "3"? "selected": "" ?> value="3">Forfait</option>
			</select>
		</div>
	</div>
	<div class="row pb-10">
		<div class="col_12 d-flex">
			<div>
				<label class="switch" style="width: 40px">
					<input class="field" id="status" type="checkbox" <?= $ppl["status"] === "1"? "checked": "" ?>>
	...
	<div class="row pb-10 pt-10">
		<div class="col_12 d-flex">
			<button class="blue store_1 mr-10" value="<?= $ppl["id_propriete"] ?>">Enregistrer</button>
			<button class="delete_1 red mr-10" value="<?=$ppl["id"]?>">Supprimer</button>
