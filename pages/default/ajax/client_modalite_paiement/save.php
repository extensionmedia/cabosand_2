...
if(file_exists($_SESSION['CORE'].$table_name.".php")){
	...
	if( $_POST["data"]["columns"]["is_default"] == 1 ){
		$dt = $ob->fetchAll("client_modalite_paiement");
		foreach($dt as $k=>$v){
			$ob->save(array("id"=>$v["id"],"is_default"=>0), "client_modalite_paiement");
		}
	}
	
	$data = array(
		"client_modalite_paiement"	=>	$_POST["data"]["columns"]["client_modalite_paiement"],
