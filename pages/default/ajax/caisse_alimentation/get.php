...
if(file_exists($core.$table_name.".php")){
	...
	$current = $p_p *  $_POST['data']['current'];
		...
		if($request == ""){
			$values = $ob->find(null,array("order"=>$sort_by,"limit"=>array($current,$p_p)),"caisse_alimentation");
			$totalItems = $ob->getTotalItems();
		}else{
			$totalItems = count($ob->find(null,array("conditions"=>array("LOWER(name) like "=>"%".$request."%"),"order"=>$sort_by),"caisse_alimentation"));
			$values = $ob->find(null,array("conditions"=>array("LOWER(name) like "=>"%".$request."%"),"order"=>$sort_by,"limit"=>array($current,$p_p)),"caisse_alimentation");
		}
