...
if(file_exists($core.$table_name.".php")){
	...
	$current = $p_p *  $_POST['data']['current'];
		...
		if($request == ""){
			$values = $ob->find(null,array("order"=>$sort_by,"limit"=>array($current,$p_p)),"v_entreprise");
			$totalItems = $ob->getTotalItems();
		}else{
			$totalItems = count($ob->find(null,array("conditions"=>array("LOWER(raison_social) like "=>"%".$request."%"),"order"=>$sort_by),"v_entreprise"));
			$values = $ob->find(null,array("conditions"=>array("LOWER(raison_social) like "=>"%".$request."%"),"order"=>$sort_by,"limit"=>array($current,$p_p)),"v_entreprise");
		}
		...
		foreach($values as $k=>$v){
			...
			foreach($columns as $key=>$value){
				...
				if(isset($v[ $columns[$key]["column"] ])){
					...
					}elseif($columns[$key]["column"] == "raison_social"){

						$returned .= "<td style='".$style."'>" . strtoupper( $v[ $columns[$key]["column"] ] ) . " <span style='color:red'>" . $v["ABR"] . "</span></td>";
