...
if(isset($_POST["id"])){
	...
	$d = $ob->fetchAll("contrat_periode");
	foreach($d as $k=>$v){
		$s = $ob->find(null,array("conditions"=>array("UID="=>$v["UID"])), "contrat");
		if(count($s) === 0){
			foreach($ob->find(null,array("conditions AND"=>array("id_periode="=>$v["id"], "source="=>"contrat")), "propriete_location") as $kk=>$vv){
				$ob->delete($vv["id"], "propriete_location");
			}
...
<div class="panel">
	<div class="panel-header" style="padding: 0px">
		<div class="panel-header-tab ">
			...
			<a href="" class="refresh_appartement" data="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>"><i class="fas fa-key"></i> Appartements</a>
	...
	<div class="panel-content" style="padding: 0px">
		<div class="tab-content">
			<div class="row  <?= strtolower($table_name) ?>" style="margin-top: 25px">
				...
				<div class="row" style="margin-bottom: 20px; padding: 0 5px">
					<div class="col_12" style="margin-bottom: 20px; text-align: right">
					
						<button class="btn btn-green periode add" value="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>"><i class="fas fa-plus-square"></i> Ajouter</button>
						<button class="btn btn-orange periode_refresh" value="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>"><i class="fas fa-sync-alt"></i></button>
					</div>
					
					<div class="contrat_periode">
						<?php 
								$UID = ($action === "edit")? $data["UID"] : substr($formToken,0,8);
							   $conditions = array("conditions"=>array("UID="=>$UID));
								require_once($core."Contrat_Periode.php"); 
							   echo $contrat_periode->drawTable(null,$conditions,null); ?>
					</div>	
			...
			<div class="location_form">
				<div class="row">
					...
					<div class="col_8-inline" style="text-align: right; padding: 10px 5px">
						<button class="btn btn-green _select_propriete" value="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>"><i class="fas fa-plus-square"></i> Ajouter</button>
						<button class="btn btn-default refresh_appartement" value="<?= ($action === "edit")? $data["UID"] : substr($formToken,0,8) ?>"><i class="fas fa-sync-alt"></i></button>
					</div>
				</div>
				
				<div class="row">
					<div class="col_12 propriete_location">
						<?php 

						require_once($core."Propriete_Location.php");
						$conditions = array("conditions"=>array("UID="=>$UID));
						$propriete_location->drawTable(null,$conditions,"v_propriete_location");
