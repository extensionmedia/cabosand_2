	...
	foreach(scandir($filesDirectory) as $k=>$v){
		...
		if($v <> "." and $v <> ".." and strpos($v, '.') !== false){
			...
			if ($ext[1] == "doc" || $ext[1] == "docx"){
				...
				$link = "http://".$statics."contrat/".$_POST['id_produit']."/".$v;
				$showImage = "showImage";
			}
			...
			$images .= "		<button style='padding:5px 7px' class='btn btn-small btn-red delete_file contrat' value='". $filesDirectory.$v."'><i class='fas fa-trash-alt'></i></button>";
			$images .= "		<button style='padding:5px 7px' class='btn btn-small btn-green edit_file contrat' value='". $filesDirectory.$v."'><i class='fas fa-edit'></i></button>";
			$link = "http://".$statics."contrat/".$_POST['id_produit']."/".$v;
			$images .= "		<button style='padding:5px 7px' class='btn btn-small btn-default download_file contrat' value='". $link ."'><i class='fas fa-download'></i></button>";
			$images .= "	</div>";
			$images .= "</div>";
