...
if(file_exists($core.$table_name.".php")){
	...
	$current = $p_p *  $_POST['data']['current'];
		$request =  strtolower( addslashes( $_POST['data']['request'] ) );
		if($request == ""){
			$values = $ob->find(null,array("order"=>$sort_by,"limit"=>array($current,$p_p)),"v_proprietaire");
			$totalItems = $ob->getTotalItems();
		}else{
			$totalItems = count($ob->find(null,array("conditions"=>array("LOWER(CONVERT(name USING latin1)) like "=>"%".strtolower($request)."%"),"order"=>$sort_by),"v_proprietaire"));
			$values = $ob->find(null,array("conditions"=>array("LOWER(CONVERT(name USING latin1)) like "=>"%". strtolower($request)."%"),"order"=>$sort_by,"limit"=>array($current,$p_p)),"v_proprietaire");
		}
				...
				if(isset($v[ $columns[$key]["column"] ])){
					if($columns[$key]["column"] == "id"){
						...
						if($v[ $columns[$key]["column"] ] == 1){
							...
							$returned .= "<td style='".$style."'><div class='label label-red'>Activé</div></td>";
						}

					}elseif($columns[$key]["column"] == "nbr_propriete"){
						$returned .= "<td style='".$style."'><span style='font-size:13px; font-weight:bold; color:blue; text-align:center'>".$v['nbr_propriete']."</span></td>";
					}else{
					...
					if($columns[$key]["column"] == "contact"){
						$returned .= "<td style='".$style."'>".$v['phone_1']."<br>".$v['phone_2']."</td>";
