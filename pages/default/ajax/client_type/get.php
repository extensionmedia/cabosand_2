...
if(file_exists($core.$table_name.".php")){
	...
	$current = $p_p *  $_POST['data']['current'];
		...
		if($request == ""){
			...
			$totalItems = count($ob->find(null,array("conditions"=>array("LOWER(client_type) like "=>"%".$request."%"),"order"=>$sort_by),""));
			$values = $ob->find(null,array("conditions"=>array("LOWER(client_type) like "=>"%".$request."%"),"order"=>$sort_by,"limit"=>array($current,$p_p)),"");
		}
