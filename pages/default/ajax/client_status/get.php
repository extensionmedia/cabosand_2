...
if(file_exists($core.$table_name.".php")){
	...
	$current = $p_p *  $_POST['data']['current'];
		...
		if($request == ""){
			$values = $ob->find(null,array("order"=>$sort_by,"limit"=>array($current,$p_p)),"");
			$totalItems = $ob->getTotalItems();
		}else{
			$totalItems = count($ob->find(null,array("conditions"=>array("LOWER(propriete_status) like "=>"%".$request."%"),"order"=>$sort_by),""));
			$values = $ob->find(null,array("conditions"=>array("LOWER(propriete_status) like "=>"%".$request."%"),"order"=>$sort_by,"limit"=>array($current,$p_p)),"");
		}
		...
		foreach($values as $k=>$v){
									...
									foreach($columns as $key=>$value){
										if(isset($v[ $columns[$key]["column"] ])){
											if($columns[$key]["column"] == "is_default"){
												$returned .= ($v[ $columns[$key]["column"] ] == 0)? "<td></td>": "<td style='font-size:10px; color:green'> <i class='fas fa-check'></i> <span>Par Défaut</span></td>";
			...
			$returned .= "<td><button style='margin-right:10px' data-page='".$table_name."' class='btn btn-red remove_ligne' value='".$v["id"]."'><i class='fas fa-trash-alt'></i></button><button data-page='".$table_name."' class='btn btn-orange edit_ligne' value='".$v["id"]."'><i class='fas fa-edit'></i></button></td>";
			$returned .= '					</tr>';
		$i++	;
		}
		if($i == 0){
